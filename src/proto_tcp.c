...
int tcp_connect_server_ft(struct connection *conn, int flags)
{
	...
	getsockname(fd, (struct sockaddr *)&connectedAddr, &connectedAddrLen);
	...
	*/
	...
	/*conn->addr_from = ntohl(((struct sockaddr_in *)&conn->addr.from)->sin_addr.s_addr);
	conn->addr_to = ntohl(((struct sockaddr_in *)&conn->addr.to)->sin_addr.s_addr);
	//add_guestip(conn->addr_to);
	struct proto_ipc *ptr = ipt_target;
	(ptr + 1)->nic[0] = conn->addr_to;
	//add_vm_target(&vm_head.vm_list, ntohl(((struct sockaddr_in *)&conn->addr.to)->sin_addr.s_addr), conn->handle.fd, conn);
	getshmid(conn->addr_from, conn->addr_to, &direction);
	printf("conn->addr_from address = %s\n", inet_ntoa((((struct sockaddr_in *)&conn->addr.from)->sin_addr)), ntohs(((struct sockaddr_in *)&conn->addr.from)->sin_port));
	printf("conn->addr_to address = %s\n", inet_ntoa((((struct sockaddr_in *)&conn->addr.to)->sin_addr)), ntohs(((struct sockaddr_in *)&conn->addr.to)->sin_port));
