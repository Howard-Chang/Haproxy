...
int set_queue_seq_HA(int fd, int queue, __u32 seq)
{
	printf("\tSetting %d queue seq to %u\n", queue, seq);
...
int get_sequence_number(int fd, struct libsoccr_sk_data* data)
{
    if (tcp_repair_on(fd) < 0) {
		return -1;
	}

    if (get_seq(fd, TCP_RECV_QUEUE, &data->inq_seq, data->inq_len))
		return -5;

	if (get_seq(fd, TCP_SEND_QUEUE, &data->outq_seq, data->outq_len))
		return -6;
	
	printf("Check seq number: inq_seq:%u  outq_seq:%u\n", data->inq_seq, data->outq_seq);
...
int get_seq(int sk, int queue_id,
		__u32 *seq, __u32 len)
{
	int ret, aux;
	socklen_t auxl;
...
int set_seq(int fd, struct sk_info *data, unsigned data_size)
{
	struct tcp_repair_opt opts[4];
	int addr_size, mstate;
	int onr = 0;
	...
	if (tcp_repair_on(fd) < 0) {
		printf("repair on fail!!!\n");
	}
	else printf("repair mode on !\n");
	...
	if (set_queue_seq_HA(fd, TCP_RECV_QUEUE,
				data->sk_data.inq_seq))
		return -2;

	seq = data->sk_data.outq_seq;


	if (set_queue_seq_HA(fd, TCP_SEND_QUEUE, seq))
		return -3;
	...
	if (data->sk_data.state != TCP_SYN_SENT &&
	    setsockopt(fd, SOL_TCP, TCP_REPAIR_OPTIONS,
				opts, onr * sizeof(struct tcp_repair_opt)) < 0) {
		printf("Can't repair options");
		return -2;
	}
	...
	if (data->sk_data.opt_mask & TCPI_OPT_TIMESTAMPS) {
		if (setsockopt(fd, SOL_TCP, TCP_TIMESTAMP,
				&data->sk_data.timestamp, sizeof(data->sk_data.timestamp)) < 0) {
			logerr("Can't set timestamp");
			return -3;
		}
	...
	if (tcp_repair_off(fd) < 0) {
		printf("repair off fail!!!\n");
	}
	return 0;
}

void free_buf(struct sk_info* buf)
{
    free(buf->recv_queue);
    free(buf->send_queue);
    free(buf);
...
int libsoccr_set_sk_data_noq_HA(struct libsoccr_sk *sk,
		struct sk_info *data, unsigned data_size, int failover)
{
	struct tcp_repair_opt opts[4];
	int addr_size, mstate;
	int onr = 0;
...
int libsoccr_restore_queue_HAProxy(struct libsoccr_sk *sk, struct sk_info *data, unsigned data_size,
		int queue, char *buf)
{
	if (!buf)
		return 0;
...
int send_fin_HAProxy(struct libsoccr_sk *sk, struct sk_info *data,
		unsigned data_size, uint8_t flags)
{
	uint32_t src_v4 = sk->src_addr->v4.sin_addr.s_addr;
	uint32_t dst_v4 = sk->dst_addr->v4.sin_addr.s_addr;
		...
		struct sk_info* data, unsigned data_size, int failover)
{
	...
	if (libsoccr_set_sk_data_noq_HA(sk, data, data_size, failover))
		return -1;
...
void get_data(struct sk_info* data_info, char* tmp, struct sk_prefix pre)
{
	...
	{
		memcpy(&data_info[i], tmp+hd_idx, sizeof(struct sk_addr) + sizeof(struct libsoccr_sk_data));	//copy header
...
struct sk_info* get_dump_info(int dt_connfd)
{
	int ret = 0;
	char recv_dt[800];
	char tmp[800];
	struct sk_info *data_info1;
	struct sk_prefix pre1;
	pre1.conn_size = 0;
    
	ret = read(dt_connfd, recv_dt, 800);
	//printf("ret:%d\n", ret);

	memcpy(tmp, recv_dt, ret);
	memcpy(&pre1, tmp, sizeof(struct sk_prefix));
	data_info1 = calloc(pre1.conn_size, sizeof(struct sk_info));
	get_data(data_info1, tmp, pre1);

	return data_info1;
}

void print_info(struct sk_info* data)
{
...
void print_sk_data_info(struct sk_data_info* data)
{
	char str[INET_ADDRSTRLEN];
...
int dump_tcp_conn_state_HA(int fd, struct libsoccr_sk_data* data, struct sk_prefix* hd, struct sk_info* buf)
{
    int ret;
...
void dump_send(int sockfd, int proxy_dt_fd, struct libsoccr_sk_data* data, struct sk_prefix* pre, struct sk_info* buf)
{
    int hd_idx, len;
...
void print_qdata(struct sk_info data_info)
{
...
void save_sk_data(struct libsoccr_sk_data* data, struct libsoccr_sk* socr, struct sk_info* buf)
{
...
void final_save_data(char *send_data, struct sk_info *buf,int hd_idx, int q_idx)
{
    memcpy(send_data+hd_idx, &buf->sk_data, sizeof(struct libsoccr_sk_data));
